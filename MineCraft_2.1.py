...
WALKING_SPEED =  PLAYER_HEIGHT + 2# 走路速度
RUNNING_SPEED = PLAYER_HEIGHT * 2 # 跑步速度
FLYING_SPEED = 15 # 飞行速度

# 35.0
GRAVITY = 35.0 # 重力
MAX_JUMP_HEIGHT = PLAYER_HEIGHT - 0.75 # 最大跳跃速度
JUMP_SPEED = math.sqrt(2 * GRAVITY * MAX_JUMP_HEIGHT)
TERMINAL_VELOCITY = 35 # 终端速度

WORLDLEN = 128 # 世界长度
 
TEXTURE_PATH = 'texture2_1.png' # 纹理文件
...
class Window(pyglet.window.Window):
    ...
    def on_key_press(self, symbol, modifiers):
        ...
        if symbol == key.W:
            ...
            #(symbol - self.num_keys[0]) % len(self.inventory)
            index = self.n
...
def main():
    window = Window(width=800, height=600, caption='Minecraft 2.1 Python Edition', resizable=True)
    window.set_exclusive_mouse(True)
    # image for icon
    img = pyglet.resource.image("Minecraft.ico")
...
if __name__ == '__main__':
    print("Welcome play python Minecraft 2.1")
    print("Snow Word is open:", GAMETYPES)
